...
async function defaultApi(apiBaseUrl: string, token?: string | null, path = "", opts: RequestInit = {}) {
  ...
  const res = await fetch(url, {
    ...
    headers: {
      ...(opts.headers || {}),
      Authorization: (token || __getIdTokenFromLocalStorage()) ? `Bearer ${token || __getIdTokenFromLocalStorage()}` : "",
...
export default function NotificationsDropdown({ api, apiBaseUrl, token }: Props) {
  ...
  const base = apiBaseUrl || (window as any).CONFIG?.API_BASE_URL || (import.meta as any)?.env?.VITE_API_URL || "";
  ...
  useEffect(() => {
    try { (window as any).__SB_NOTIF_VARIANT = 'Dropdown-v3'; console.log('Notifications component mounted: Dropdown-v3'); } catch {}
    load(false);
  ...
  async function declineFollow(n: NotificationItem) { console.log('Decline clicked (mdown)', n); console.log('Decline clicked', n);
    try {
      const body = { fromUserId: n.fromUserId, requesterId: n.fromUserId };
      const res = await call("/follow-decline", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        alert(`Decline failed: ${res.status} ${txt?.slice(0,140)}`);
        return;
      }
      setItems((prev) => prev.filter((x) => x.id !== n.id));
    } catch (e) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        \1<span className="sr-only"> Dropdown-v3</span>
        {unread > 0 && (
  ...
  (n.message || "sent you a notification")
                    ...
                    {n.type === "follow_request" && (
                      <div className="flex gap-1">
                        <button
                          onMouseDown={(e)=>{ e.preventDefault(); e.stopPropagation(); acceptFollow(n); }} type="button"
                        ...
                        <button
                          onMouseDown={(e)=>{ e.preventDefault(); e.stopPropagation(); declineFollow(n); }} type="button"
